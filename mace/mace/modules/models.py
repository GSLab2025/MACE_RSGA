...
from mace.modules.rsga import ReciprocalSpaceGatedAttention
...
class MACERSGA(MACE):
    def __init__(
        ...
        **kwargs,
        ...
        self.silu = torch.nn.SiLU() #RSA
        #self.sigmoid = torch.nn.Sigmoid() #RSA
        #self.alpha_lr_sr = torch.nn.Parameter(torch.tensor(0.0, dtype=torch.get_default_dtype())) #RSA
        
        self.rsga = torch.nn.ModuleList()

        hidden_irreps = kwargs.get('hidden_irreps') 
        ...
        for i in range(self.num_interactions):
            ...
            self.rsga.append(
                ReciprocalSpaceGatedAttention(node_irreps=layer_irreps,
                            r_max=self.r_max,
                            hidden=None)
            )
    ...
    def forward(
        ...
        # Interactions
        node_es_list: List[torch.Tensor] = [] # [pair_node_energy] ;  previously the ZBL replusion energy was initialized!
        ...
        for i, (interaction, product) in enumerate(
            ...
            # --- RSA: Clone features BEFORE interaction ---
            #node_feats_x = node_feats.clone() # pre-interaction
            ...
            #Per Layer RSA
            node_feats_lr, gate_sr_lr = self.rsga[i](
                 data,
                 node_feats,                  # (N, F); previously node_feats_x
            )
            #lr_gate = self.sigmoid(self.alpha_lr_sr) 
            node_feats =  node_feats +  gate_sr_lr  * node_feats_lr  # Residual connection ;broadcasts over feature dim
        ...
        node_inter_es_learned = torch.sum(torch.stack(node_es_list, dim=0), dim=0)
        node_inter_es_learned = self.scale_shift(node_inter_es_learned, node_heads)

        # Add ZBL AFTER scale/shift
        node_inter_es_total = node_inter_es_learned + pair_node_energy
        #pr = pair_node_energy
        #print(
        #"pair_node_energy stats:",
        #"min", pr.min().item(),
        #"max", pr.max().item(),
        #"mean", pr.mean().item(),
        #"std", pr.std().item(),
        #"absmax", pr.abs().max().item(),
        #"dtype",pr.dtype,
        #"shape",pr.shape)

        inter_e = scatter_sum(node_inter_es_total, data["batch"], dim=-1, dim_size=num_graphs)
        #Previously scaleshift was applied to ZBL also, infact everything summed and scale shifted! So this reduced replusion magnitude!
        #node_feats_out = torch.cat(node_feats_list, dim=-1)
        ...
        node_energy = node_e0.double() + node_inter_es_total.double()
